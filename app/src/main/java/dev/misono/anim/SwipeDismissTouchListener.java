...
public class SwipeDismissTouchListener implements View.OnTouchListener {
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (dismiss) {
				// dismiss
				ViewCompat.animate(mView).translationX(dismissRight ? mViewWidth : -mViewWidth).alpha(0)
						.setDuration(mAnimationTime).setListener(new DefViewPropertyAnimatorListener() {
							@Override
				...
				// cancel
				ViewCompat.animate(mView).translationX(0).alpha(1).setDuration(mAnimationTime).setListener(null);
			}
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (mSwiping) {
				...
				ViewCompat.setAlpha(mView, Math.max(0f, Math.min(1f, 1f - 2f * Math.abs(deltaX) / mViewWidth)));
				return true;
			}
			break;
		}
