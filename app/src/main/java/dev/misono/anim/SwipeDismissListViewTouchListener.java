	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_CANCEL: {
			...
			if (mDownView != null && mSwiping) {
				...
				// mDownView.animate()
						.translationX(0).alpha(1).setDuration(mAnimationTime).setListener(null);
			}
		...
		case MotionEvent.ACTION_UP: {
			...
			if (dismiss && mDownPosition != ListView.INVALID_POSITION) {
				...
				// cancel
				ViewCompat.animate(mDownView).translationX(0).alpha(1).setDuration(mAnimationTime).setListener(null);
			}
			mVelocityTracker.recycle();
			mVelocityTracker = null;
			mDownX = 0;
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (mSwiping) {
				...
				ViewCompat.setAlpha(mDownView, Math.max(0f, Math.min(1f, 1f - 2f * Math.abs(deltaX) / mViewWidth)));
				return true;
			}
			break;
		}
